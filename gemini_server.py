...
## 【絶対厳守】パラメータ反映ルール
- **絶対決める項目**：参考漫才師、A、B、テーマは必ず反映
- **オプション項目**：未指定の場合は「{指定なし}」をそのまま出力し、絶対に補完・展開しない
- **話者名**：A/Bが役割指定の場合は「A」「B」を使用し、A/Bの名前が指定された場合は必ずその名前を台本の話者名として使用すること（A/B表記は使わず、指定された名前のみを使う）。
  （例：Aの名前が「たろう」、Bの名前が「じろう」の場合、台本のセリフは「たろう: ...」「じろう: ...」の形式で出力すること）
  また、A/Bの名前の指定がない場合は絶対に「A」「B」のまま出力し、他の名前や表記に変更・補完しないこと。

## 【必須】セリフ形式
- セリフは必ず「A: 内容」「B: 内容」または指定された話者名: 内容 の形式（A/Bの名前が指定された場合は必ずその名前で出力し、指定がない場合は必ず「A」「B」のまま出力すること）
- 1セリフごとに必ず改行
...
def gemini_api():
    ...
    # 絶対決める項目のバリデーション（フロントで済んでいるが念のため）
    # 参考漫才師, A, B, テーマが含まれているかチェック
    required_keys = ["参考漫才師:", "A:", "B:", "テーマ:"]
    for k in required_keys:
        if k not in user_input:
            return jsonify({"error": f"絶対決める項目({k})が入力されていません。"}), 400
